namespace CoreBanker.Services
{
    public class EndOfDayService : ApiClientBase
    {
        ...
        {
            return await GetAsync<EodStatusDto>("/api/operations/eod/status", cancellationToken);
        }

        public async Task<EodStepResultDto?> RunStepAsync(RunEodStepRequest request, CancellationToken cancellationToken = default)
        {
            return await PostAsync<RunEodStepRequest, EodStepResultDto>("/api/operations/eod/run-step", request, cancellationToken);
        }
    }
